          ...
          API.get(`/users/${userId}`).then((response) => {
            if (response.status === 200 && response.data.success) {
              setOrderForm((prev) => ({
                ...
                phoneNum: response.data.content.phoneNum || "",
                address: response.data.content.address || "",
                zipcode: response.data.content.zipcode || "",
                addrDetail: response.data.content.addrDetail || "",
              }));
            }
          });
        } catch (err) {
    ...
    async (page) => {
      ...
      try {
        ...
        const response = await API.get(
          `/orders?page=${page}&size=${pageSize}&id=${userId}`
        );

        if (response.status === 200) {
          console.log("주문 목록:", response.data);
          setOrders(response.data.content.list || []);
          setTotalPages(response.data.content.totalPage || 0);
        } else {
      ...
      if (response.status === 200 || response.status === 201) {
        ...
        if (location.state?.fromCart) {
          ...
          const updatedCart = cartItems.filter(
            (item) => !orderItems.some((orderItem) => orderItem.id === item.id)
          );
                ...
                <div className="order-header">
                  ...
                  <div
                    className={`order-status ${
                      orderStatusMap[order.orderStatus]?.className || ""
                    }`}
                  >
                ...
                <div className="order-info">
                  <div className="order-details">
                    ...
                    <div className="order-payment">
                      결제방법: {payTypeMap[order.payType] || order.payType}
                    </div>
                    <div className="order-amount">
                      결제금액: {order.totalPrice.toLocaleString()}원
                    </div>
                    <div className="order-items-count">
                      상품 개수: {order.totalCount}개
                    </div>
                  </div>
