...
function BoardDetail({ apiEndpoint = '/board' }) {
    ...
    // API 엔드포인트 정규화
    const normalizedApiEndpoint = apiEndpoint.startsWith('/') ? apiEndpoint : `/${apiEndpoint}`;
    ...
    const handleDelete = async () => {
        ...
        try {
            ...
            alert('게시물 삭제 중 오류가 발생했습니다: ' + (err.message || '알 수 없는 오류'));
        }
    };
    ...
    if (error) {
        return (
            <div className="error-container">
                ...
                <button className="back-button" onClick={handleGoBack}>목록으로 돌아가기</button>
            </div>
        );
    }
        ...
        <div className="board-detail-container">
            ...
            <div className="post-detail-header">
                ...
                <div className="post-detail-meta">
                    {post.region && <span className="post-detail-region">{post.region}</span>}
                    <span className="post-detail-author">{post.name || '작성자 없음'}</span>
                    <span className="post-detail-date">
                        {post.createdAt ? new Date(post.createdAt).toLocaleDateString() : '날짜 정보 없음'}
                    </span>
                </div>
            </div>
