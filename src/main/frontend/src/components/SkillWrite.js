...
function SkillWrite({ apiEndpoint = "/skill", title = "기술 작성" }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (apiEndpoint === "/skill") {
        ...
        skillFormData.append("request", jsonBlob);
        // 선택한 파일이 있는 경우 FormData에 추가
        if (selectedFiles.length > 0) {
          selectedFiles.forEach((file) => {
            skillFormData.append("images", file); // 'images' 키로 파일 추가
          });
        }

        console.log("기술 등록 요청 데이터:", skillFormData);

        // 백엔드 API 서버 URL 직접 지정 (CORS 이슈가 있을 수 있음)
        // 개발환경 프록시 설정 확인 필요

        response = await API.post(`${apiEndpoint}`, skillFormData, {
          headers: {
        ...
        if (isEditMode) {
          response = await API.put(`${apiEndpoint}/${id}`, skillFormData, {
            headers: {
              "Content-Type": "multipart/form-data",
            }
        });
